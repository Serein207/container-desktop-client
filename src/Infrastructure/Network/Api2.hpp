...
struct Api2 {
    ...
    static std::tuple<QNetworkRequest, QByteArray>
        makeRequest(ContainerDesktopDetails::Method verb, const QUrl& url,
        ...
        auto header =
            QJsonObject{{"PVEAuthCookie",
                         getCookieOrDefault(cookies, "PVEAuthCookie", "")}};
        dataObject.insert("header", header);

        for (auto it = header.begin(); it != header.end(); ++it) {
            if (!containsCookie(cookies, it.key())) {
                QNetworkCookie cookieItem(it.key().toUtf8(),
